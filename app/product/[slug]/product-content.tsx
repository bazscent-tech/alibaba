      ...
      <main className="shell product-page animate-fade-in">
        ...
        <div className="product-layout">
          ...
          <section className="product-info">
            ...
            {supplier ? <div className="supplier-panel"><div className="supplier-panel__avatar">{supplier.name.slice(0, 1)}</div><div className="supplier-panel__copy"><div><Link href={`/store/${supplier.id}`} className="supplier-panel__store-link"><strong>{supplier.name}</strong></Link>{supplier.verified ? <CheckCircle className="h-3.5 w-3.5 text-[#8d2941]" /> : null}</div><p><MapPin className="h-3 w-3" /> {supplier.country} <i /> {supplier.yearsInBusiness} سنوات خبرة</p></div><Link href={`/messages?supplier=${encodeURIComponent(supplier.name)}&product=${encodeURIComponent(product.name)}`} className="supplier-panel__contact"><MessageCircle className="h-4 w-4" /> تواصل</Link></div> : null}
